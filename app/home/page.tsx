...
export default function HomePage() {
  ...
  return (
    <div className="min-h-dvh bg-[#0D0D0D]">
      ...
      {/* ── NAVIGATION ───────────────────────────────────────────────── */}
      <header className="sticky top-0 z-30 bg-[#0D0D0D]/95 backdrop-blur-sm border-b border-[#1A1A1A]">
        <div className="page-container h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {streak > 0 && (
              <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-[#171717]">
            ...
            <button
              ...
              className="w-9 h-9 rounded-xl bg-[#171717] hover:bg-[#1E1E1E] flex items-center justify-center font-black text-sm text-white transition-colors"
            >
              {avatar}
            </button>
          </div>
      ...
      <motion.div
        ...
        className="page-container py-10 space-y-10"
      >

        {/* ── DAILY CHALLENGE ────────────────────────────────────────── */}
        <motion.section variants={up}>
          <p className="font-game text-[#444] text-sm mb-4" style={{ letterSpacing: '0.2em' }}>
            DAILY CHALLENGE
          </p>

          <div
            onClick={() => !isCaseDone && router.push('/daily')}
            className={`rounded-2xl bg-[#111111] border border-[#1A1A1A] overflow-hidden ${!isCaseDone ? 'cursor-pointer' : ''}`}
          >
              ...
              {(['linkGrid','timeTrace','trueLie','codeBreak'] as PuzzleType[]).map((t, i) => {
                ...
                return (
                  <motion.div
                    key={t}
                    className="flex-1"
                    style={{ background: s === 'solved' ? PUZZLE_COLOR[t] : s === 'active' ? `${PUZZLE_COLOR[t]}55` : '#1A1A1A' }}
            ...
            <div className="p-8 lg:p-10 flex flex-col md:flex-row md:items-center gap-8">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-3">
                  <span className="text-xs font-bold text-[#444] uppercase tracking-[0.2em]">
                ...
                <h2 className="font-game text-white mb-6"
                    ...
                    {(['linkGrid','timeTrace','trueLie','codeBreak'] as PuzzleType[]).map((t, i) => {
                      ...
                      return (
                        <div
                          ...
                          style={{
                            background: s === 'solved' ? `${PUZZLE_COLOR[t]}20` : '#1A1A1A',
                            color: s === 'solved' ? PUZZLE_COLOR[t] : '#444',
                            border: `1px solid ${s === 'solved' ? `${PUZZLE_COLOR[t]}40` : '#222'}`,
                          }}
                        >
                          {s === 'solved' && <span>✓</span>}
                          {meta.label}
                ...
                {isCaseDone ? (
                  <div className="flex items-center gap-6">
                    <div>
                      <p className="text-xs text-[#444] uppercase tracking-widest mb-1">Final Score</p>
                    ...
                    <button
                      ...
                      className="px-5 py-3 rounded-xl bg-[#1E1E1E] text-white text-sm font-bold hover:bg-[#252525] transition-colors"
                    >
                      View Result →
                    </button>
                  </div>
              ...
              {/* Right: solved indicator */}
              <div className="shrink-0 hidden lg:flex">
                {isCaseDone ? (
                  <div
                    className="w-24 h-24 rounded-2xl flex items-center justify-center"
                    style={{ background: '#C8FF5720', border: '1.5px solid #C8FF5740' }}
                  >
                    <span className="font-game text-[#C8FF57] text-5xl">✓</span>
                  </div>
                ) : (
                  <div className="w-24 h-24 rounded-2xl bg-[#1A1A1A] flex items-center justify-center">
            ...
            {puzzleTypes.map((type, i) => {
              ...
              return (
                <motion.button
                  ...
                  className="group text-left rounded-2xl bg-[#111111] border border-[#1A1A1A] p-6 transition-all hover:border-[#2A2A2A]"
                >
                  <div className="mb-5">
                    <PuzzleIcon type={type} size={32} />
                  </div>
                  <p className="font-game text-white text-2xl mb-1" style={{ letterSpacing: '0.04em' }}>
                    {meta.label.toUpperCase()}
                  </p>
                  <p className="text-xs text-[#555] leading-relaxed mb-4">{meta.description}</p>
                  <span
                    className="inline-block text-[10px] font-black px-2 py-1 rounded-md uppercase tracking-wider"
        ...
        <motion.section variants={up}>
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              { label: 'Streak',     value: streak,      unit: 'days',   color: '#F97316', icon: <FlameIcon  size={18} color="#F97316" /> },
              { label: 'Best Score', value: bestScore,   unit: 'points', color: '#C8FF57', icon: <StarIcon   size={18} color="#C8FF57" /> },
              { label: 'Solved',     value: totalSolved, unit: 'cases',  color: '#8B5CF6', icon: <TrophyIcon size={18} color="#8B5CF6" /> },
            ].map(s => (
              <div key={s.label} className="rounded-2xl bg-[#111111] border border-[#1A1A1A] p-6 flex items-start gap-4">
                <div className="mt-1">{s.icon}</div>
                <div>
                  <p className="font-game leading-none mb-1" style={{ fontSize: '40px', color: s.color }}>
                    {s.value.toLocaleString()}
                  </p>
                  <p className="text-xs text-[#555] uppercase tracking-wider">{s.label}</p>
                </div>
              </div>
            ))}
          </div>
